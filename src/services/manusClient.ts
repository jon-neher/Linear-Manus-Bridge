...
const log = createLogger('manusClient');
...
async function requestCreateTask(
  ...
  try {
    const response = await fetchWithTimeout(`${MANUS_API_BASE_URL}/v1/tasks`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      log.error({ status: response.status, text: text.slice(0, 500) }, 'requestCreateTask failed');
      return { ok: false, status: response.status, text };
    }

    const data = (await response.json()) as ManusTaskResponse;
    log.info(
      { taskId: data.task_id, taskUrl: data.task_url ?? '(none)' },
      'requestCreateTask success'
    );
    return { ok: true, data };
  } catch (error) {
    if (isTimeoutError(error)) {
      const timeoutMsg = handleTimeoutError('requestCreateTask', error);
      log.error({ error: timeoutMsg }, 'requestCreateTask timeout');
      return { ok: false, status: 504, text: timeoutMsg };
    }
    throw error;
  }
}
...
export async function replyToTask(
  ...
  try {
    ...
    if (isTimeoutError(error)) {
      throw new Error(handleTimeoutError('replyToTask', error));
    }
    throw error;
  }
}
...
export async function createFileRecord(filename: string): Promise<ManusFileResponse> {
  ...
  try {
    const response = await fetchWithTimeout(`${MANUS_API_BASE_URL}/v1/files`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        API_KEY: apiKey,
    ...
    if (isTimeoutError(error)) {
      throw new Error(handleTimeoutError('createFileRecord', error));
    }
    throw error;
  }
}
...
export async function uploadFileToManus(
  ...
  try {
    ...
    if (isTimeoutError(error)) {
      throw new Error(handleTimeoutError('uploadFileToManus', error));
    }
    throw error;
  }
}
